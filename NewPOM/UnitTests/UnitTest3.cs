...
namespace NewPOM.UnitTests
{
    ...
    class UnitTest3 :BaseFixture
    {
        ...
        {
            string step = "";
            //_test = extent.CreateTest(NUnit.Framework.TestContext.CurrentContext.Test.MethodName); //this.GetType.ToString()
            try
            {
            ...
            catch (Exception e)
            {
                ...
                MediaEntityModelProvider mediaEntity = MediaEntityBuilder.CreateScreenCaptureFromBase64String(screenshot, step).Build();
                ExtentTestCustom.GetTest().Fail(step + "\n\n" + e.StackTrace + "\n\n", mediaEntity);
                Assert.Fail();
            }
